...
`BroadcastChannel` looks good method for the purpose, but there is a restriction: it cannot bridge between tabs with different containers. Only default container tab can commucate with the background script, and other container tabs cannot. If you don't need to use non-default container tabs, see [Cross Context Messaging](https://github.com/piroor/webextensions-lib-cross-context-messaging) built on `BroadcastChannel`.
